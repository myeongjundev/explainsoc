...
describe('논문 예시로 60초 검토', () => {
  it('결과로 바로 가서 R04와 그 질문을 보여 준다', async () => {
    ...
    expect(screen.getByRole('heading', { level: 2, name: '3. 결과와 질문' })).toHaveFocus()
    expect(screen.getByText(/R04/)).toBeInTheDocument()
    const questions = screen.getByRole('region', { name: '공급자에게 물을 질문' })
    expect(within(questions).getAllByRole('listitem').map((li) => li.textContent)).toEqual(['같은 시험에서 공격 Recall은 얼마입니까?'])
  })
...
describe('입력 검증 (BRB-C05)', () => {
  ...
  it('잘못된 입력 하나가 다른 유효한 입력의 결과를 막지 않는다', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /결과와 질문/ }))
    expect(screen.getByText(/R04/)).toBeInTheDocument()
...
describe('질문 복사', () => {
  it('질문 문장만 복사하고 입력한 숫자는 담지 않는다', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /결과와 질문/ }))
    await user.click(screen.getByRole('button', { name: '질문 복사' }))
