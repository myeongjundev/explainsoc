...
| 12번 카드 | `about` 커밋 `fbf5948` — 요약과 사례 노트(문제·판단·구현·검증)를 README 30초 요약에 맞춤. 그림 `docs/assets/t13-conclusion-v9.png`·링크·152개·35개는 그대로 |
...
| 항목 | V10 결론 수정판 제출본 |
|---|---|
| 기준 커밋 | `3ae1312e103b3fd494594b9c8f00d05d4aa07eb5` (독립 검토 수정 후보 1) |
| 배포 | GitHub Actions [run 36021634394](https://github.com/myeongjundev/explainsoc/actions/runs/36021634394) 검사·Pages 성공 |
| 공개 주소 | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-3ae1312.zip` · 196,359 bytes · 71개 파일 |
| SHA-256 | `e0429ac2e1696c3fd95b791e11a279ed776bdf0feabe46e8d9f1d0bb6c511ac6` |
| 재현 | 같은 커밋으로 두 번 생성한 SHA-256 일치 |
| 빈 폴더 검증 | 저장소 밖 새 임시 폴더에서 `npm ci`(취약점 0), Vitest 152개, 프로덕션 빌드, 안전 검사 11개, Playwright 35개 통과(증거 캡처 26개 건너뜀) |
| 공개본 검증 | JS `index-DfnmrLxk.js`, CSS `index-DhJb78PJ.css` 로컬 빌드와 SHA-256 일치 · 공개 주소에서 브라우저 검사 35개 통과 |
| 12번 카드 | `about` 커밋 `b4084d4` — 그림 `docs/assets/t13-conclusion-v9.png` 그대로, 검증 문장 152개·35개 |

화면 구성은 V10과 같다. 바뀐 것은 결론 한 갈래다. "시험에 학습 때 없던 공격이 들어 있었다"는 답만 있고 분할 방식이
"따로 시험"이 아니면, 결론이 더는 "처음 보는 공격으로 시험한 숫자가 있습니다"라고 말하지 않는다. 대신 그 공격만의
성능을 따로 확인한 숫자인지는 알 수 없다고 말한다. 근거는 `planning/CODEX-V10-INDEPENDENT-REVIEW-HANDOFF.md` 4절이다.

## V10 품질 개선판 제출본 — 2026-09-24

> 이 섹션은 위 V10 결론 수정판 제출본으로 대체됐다. 아래 V9 제출본의 실행 묶음과 검증 값을 대체했던 기록이다.
...
| 항목 | V7 제출본 |
|---|---|
| 구현 커밋 | `4489803e2a4d5b79f64c1cec235ed4d5f34baa65` |
| 배포 | GitHub Actions `Verify and deploy to GitHub Pages` 검사·Pages 성공 |
| 공개 주소 | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-4489803.zip` · 145,603 bytes · 64개 파일 |
| SHA-256 | `3eca59e1007b9767ef5bd4f0953f098feb048ced5fc34be4aa5623e0536c53dc` |
| 재현 | 같은 커밋으로 두 번 생성한 SHA-256 일치 |
| 빈 폴더 검증 | 저장소 밖 새 임시 폴더에서 `npm ci`(취약점 0), Vitest 95개, 프로덕션 빌드, 안전 검사 11개, Playwright 26개 통과 |
| 공개본 검증 | JS `index-BkE6dFvd.js`, CSS `index-CJSslC0U.css` 로컬 빌드와 SHA-256 일치 · 공개 주소에서 브라우저 검사 26개 통과 |

V7은 앱을 처음 본 두 사람의 관찰(결과 화면이 밀집해 읽기 힘들다, 그림이 있으면 좋겠다)을 반영했다.
결과를 `주장과 근거 / 판독 / 다음 행동` 세 장으로 나눠 **한 번에 한 장만** 보이게 했고(1280px에서
5.2화면·패널 16개 → 장마다 3.5~4.6화면), 장 아래에 `이전 / 다음`과 `N / 3` 위치를 두었다. 받은
혼동행렬은 `공격 220,788건 가운데 탐지 160건 · 0.072%`, `정상 375,518건 가운데 오탐 86건 · 0.023%`를
실제 비율 막대로 그린다. 축을 자르거나 최소 너비를 주지 않아 탐지 막대가 거의 보이지 않으며, 그
사실을 눈금과 글로 알린다. 캐릭터·삽화는 넣지 않았다. 수치·논문 근거·R01~R14·JSON·보안 경계는
그대로다. 설계 근거는 `planning/DESIGN.md` 29절이다.

V7 증거 화면은 `evidence/v7-chapter-evidence-desktop.png`, `evidence/v7-chapter-evidence-mobile.png`,
`evidence/v7-scale-figure-desktop.png`에 있다.
...
> 2026-09-25: BRB-C11은 여섯 번째 관찰로 채워졌다. 남은 것은 사람이 잰 60초와 BRB-C14 ②다.
...
### 두 번째 관찰 — 결과 화면이 밀집해 읽기 힘들다

2026-09-23 사용자가 V6를 두 사람에게 보여 주고 전달한 관찰이다. 기기·브라우저, 걸린 시간, 세 행동
완주 여부, 직접 인용은 받지 않았다. 따라서 이 관찰도 BRB-C11 완료 증거가 아니다.

| 항목 | 기록 |
|---|---|
| 대상 | 앱을 처음 본 2명 · 이름 기록 안 함 |
| 막힌 곳 | 버튼을 눌러 다음으로 넘어간 뒤 보이는 것들이 너무 밀집해 보기 힘들다 |
| 요청 | 캐릭터나 그래프, 보이는 이미지가 있으면 더 좋겠다 |
| 세 행동·60초 경로 | 확인하지 못함 |

Claude가 같은 화면을 직접 재어 관찰을 뒷받침했다: 1280px 결과 화면은 5.2화면 길이에 패널 16개를
세 열로 동시에 펼친다. 이 사실을 근거로 V7에서 결과를 한 번에 한 장만 보이게 바꾸고, 받은 혼동행렬을
실제 비율 막대로 그렸다(`DESIGN.md` 29절). 캐릭터는 넣지 않기로 사용자와 정했다. 도구의 성격을 바꾸고,
숫자 자체를 설명하지 못하기 때문이다.

### 세 번째 관찰 — 첫 화면이 전문적이라 쓰지 않을 것 같다

2026-09-24 사용자가 V9 작업본(`v9-brochure` 브랜치, 사용자 PC의 로컬 미리보기)을 비전공 지인 한 명에게 PC로
보여 주고 전달한 관찰이다. 반응은 첫 화면에서 나왔다. 걸린 시간, 버튼을 눌러 봤는지, 세 행동 완주 여부는
기록되지 않았다. 따라서 BRB-C11 완료 증거가 아니다.

| 항목 | 기록 |
|---|---|
| 대상 | 비전공 지인 1명 · 이름 기록 안 함 |
| 기기·화면 | PC 브라우저 · V9 작업본 첫 화면(`localhost` 미리보기) |
| 사용자가 전한 반응 | “이게 뭔데? 어떻게 하는 건데? 음 잘 모르겠는데? 뭔가 전문적인 거 같은데 난 잘 몰라서 사용하지 않을 것 같아.” |
| 막힌 곳 | 첫 화면에서 무엇을 하는 사이트인지, 어떻게 쓰는지 알지 못함 |
| 세 행동·60초 경로 | 확인하지 못함 |

첫 관찰(V6 전)과 같은 신호가 첫 화면 개편 뒤에도 다시 나왔다. 화면 모양보다 첫 화면의 말(분할, Macro F1,
미관측 공격 스트레스 테스트 같은 용어)과 버튼 세 개가 막고 있다고 본다. 이 사람은 앱의 대상(보안 담당자)이
아니지만, 포트폴리오를 보는 채용 심사자도 비전공자일 수 있어 첫 화면은 누구나 10초 안에 "무엇을 해 주는지"
알아야 한다. 이 근거로 V9에서 첫 화면을 시험 공부 비유로 다시 쓴다(`DESIGN.md` 31절).

### 네 번째 관찰 — 결과가 좋은지 나쁜지 알 수 없다, 그래서 뭐?

2026-09-24 첫 화면을 시험 공부 비유로 다시 쓴 뒤(`d62ee09`) 사용자가 비전공 지인에게 다시 보여 주고 전달한
반응이다. 어디까지 눌러 봤는지, 기기, 걸린 시간은 사용자도 확인하지 못했다(“잘 모름”). 결과 화면을 본 것은
반응 내용으로 알 수 있다. BRB-C11 완료 증거가 아니다.

| 항목 | 기록 |
|---|---|
| 대상 | 비전공 지인 · 이름 기록 안 함 |
| 본 화면 | V9 작업본 첫 화면과 결과 화면 · 어떤 입력으로 결과를 냈는지는 모름 |
| 사용자가 전한 반응 | 이게 뭐 하는 사이트인지 잘 모르겠다. 결과가 나왔는데 글만 읽어서는 좋은 건지 나쁜 건지 판단이 안 되고, 결과가 어떤 건지, 그래서 뭐? |
| 막힌 곳 | 결과 첫머리에 결론이 없다(“1개의 해석 주의를 먼저 읽어야 합니다”는 무엇을 하라는지 말하지 않는다) |

사용자가 세 가지 제안(첫 화면 질문형 제목, 결과 결론 카드, 99.88% 예시로 시작) 가운데 **결론 카드만** 넣기로
정했다. 결론 카드는 AI 제품을 좋다·나쁘다로 판정하지 않고, 광고 숫자만으로 믿을 근거가 있는지와 받은 숫자가
말하는 사실을 쉬운 말로 먼저 보인다(`DESIGN.md` 31절).

### 다섯 번째 관찰 — 보안 AI 광고를 본 적이 없다

2026-09-24 결론 카드를 넣은 뒤(`1e514e9`) 같은 날 사용자가 전한 비전공 지인의 말이다. 기기·시간·세 행동은
기록되지 않았다. BRB-C11 완료 증거가 아니다.

| 항목 | 기록 |
|---|---|
| 대상 | 비전공 지인 · 이름 기록 안 함 |
| 사용자가 전한 말 | 살면서 “보안 AI의 99점” 같은 광고를 잘 본 적이 없다 |
| 뜻 | 첫 화면의 출발 상황(보안 AI 광고)이 일반인에게는 낯설다. 이 앱의 대상은 보안 담당자라 일반인이 쓰지 않는 것은 문제가 아니지만, 첫 화면은 채용 심사자를 포함한 누구나 알아들어야 한다 |

이 앱의 상황(보안 AI 제품 자료를 받아 검토하는 일) 자체가 비전공자에게는
겪어 본 적 없는 일이라는 뜻이다. BRB-C11의 대상은 “처음 보는 동료”이므로, 공식 재시험은 앱의 대상에 가까운
사람(예: 학원 동기)에게 한다.

### 여섯 번째 관찰 — V10 공개 사이트, 혼자 잘했다

2026-09-25 사용자가 전한 결과다. 사용자가 공개 사이트를 동기에게 보여 줬고, 특별히 막히거나 헷갈려하지 않았다고
했다. 뒤이어 사용자가 양식 항목을 채워 주었고(아래 표), 처음에는 기억하지 못했던 한 줄 감상도 같은 날 전해 주었다.
처음 보는 동료 한 사람의 한 줄 감상이 이름 없이 적혔으므로 **BRB-C11을 완료로 표시한다.** 한 줄은 사용자가 전한
문장을 그대로 옮겼다.
...
| 대상 | 동기 · 이름 기록 안 함 |
| 기기·브라우저 | 휴대폰 크롬 |
| 1분 안에 무슨 앱인지 말했나 | 예 (초 단위 시간은 기록 없음) |
| 업체 질문까지 도움 없이 갔나 | 예 |
| 막힌 곳 | 없음 |
| 사용자가 전한 말 | "별 반응 없던데", "혼자 잘하던데" |
| 이름 없는 한 줄 | "아, 보안 AI 업체가 99%라고 했을 때 그 숫자를 그냥 믿지 말고 어떤 테스트인지 확인하게 해주는 거구나" |
| 버전 | 기록 없음 (같은 날 공개 사이트는 `3ae1312`) |

이 관찰로 독립 검토 후보 2(모바일 결과에서 `업체에 물을 질문 보기`를 결론 제목 가까이 옮기기)는 **하지 않는다**.
휴대폰에서 도움 없이 업체 질문까지 갔으므로, 인계 문서 5절의 조건("신규 사용자가 자연스럽게 질문까지 도달하면
현재 배치를 유지")에 맞춰 현재 배치를 유지한다.
...
실행 묶음: explainsoc-4489803.zip — README.md "실행"대로 npm ci → npm run build → npm run preview 후
...
- 묶음 안: src/data/paperEvidence.ts(논문 근거 P01~P10과 수치), src/domain/reviewRules.ts(판독 규칙
  R01~R14), tests/(단위·컴포넌트 95개, 브라우저 26개)

세 행동 해 보기
0) 결과는 한 번에 한 장만 보인다. 위쪽 REVIEW MAP이나 아래 "다음" 단추로 주장과 근거 → 판독 →
   다음 행동을 차례로 연다.) "논문 예시로 60초 검토"를 누르거나, "내 성능표 검토"에서 지표 하나(예: 정확도 0.99)를 적는다.
...
무엇이 나오면 통과
- 논문 예시: "공격 220,788건 가운데 탐지 160건"과 "이 시험의 크기를 그대로 그리면" 막대(탐지 0.072%)와 공급자 질문 "같은 시험에서 공격 Recall은
  얼마입니까?"가 나온다.
...
| 실행 묶음 | `explainsoc-4489803.zip` · 145,603 bytes · 64개 파일 · SHA-256 `3eca59e1007b9767ef5bd4f0953f098feb048ced5fc34be4aa5623e0536c53dc` · 원본 커밋 `4489803e2a4d5b79f64c1cec235ed4d5f34baa65` · 로컬 저장소 `release/` |
...
현재 제출물은 V7 구현 커밋으로 만든 **`explainsoc-4489803.zip`**이다. 저장소 밖 새 임시 폴더에서
...
2026-09-25 제출본(`3ae1312`) 기준으로 위 7개를 다시 열어 모두 200 · 리디렉션 0을 확인했다. 아래 문단은 V6 당시 기록이다.
...
R01~R15, 접근성·보안 검증, GitHub Pages 배포, 재현 가능한 ZIP과 카드별 증거 기록을 맡겼다. 현재
제출본(`3ae1312`)은 단위·컴포넌트 152개와 실제 브라우저 35개를 통과했다.

한 줄 제출안: **과제·논문 분석부터 제품 설계, React 구현, 판독 규칙 R01~R15와 접근성·보안 검증,
Pages 배포와 재현 가능한 ZIP·증거 기록까지 Codex와 Claude에게 맡겼다.**

### ② 내가 직접 판단한 일

> 2026-09-25 사용자 요청으로 Claude가 `planning/STATUS.md`와 이 문서에 남은 **사용자 결정만** 모은 초안이다.
> 사용자가 읽고 자기 말로 고친 뒤 제출한다. 기록에 없는 판단은 넣지 않았다.

| 날짜 | 내가 정한 것 | 기록 |
|---|---|---|
| 09-21 | 논문 숫자를 보여 주는 앱이 아니라, 받은 성능 숫자에서 빠진 시험 조건을 찾아 공급자 질문으로 바꾸는 판독기로 방향과 설계를 승인했다. 제품을 합격·불합격으로 판정하지 않는다 | `STATUS.md` 09-21, `DESIGN.md` |
| 09-22 | 처음 보는 비전공자에게 공개 주소를 직접 보여 줬고, 목적이 전달되지 않았다는 결과를 숨기지 않고 첫 화면을 고치게 했다. 코드 라이선스는 MIT로 정했다 | `STATUS.md` 09-22, 이 문서 첫 관찰 |
| 09-23 | 결과 1장에 두 번 나오던 근거 패널을 하나로 합치기로 했다. "그림이 있으면 좋겠다"는 반응에도 캐릭터·삽화는 넣지 않았다 | `STATUS.md` 09-23 |
| 09-24 | 동기들의 앱을 보고 새 프로젝트를 만들지 않고 이 앱을 키우기로 했다. "그래서 뭐?" 반응에 나온 세 제안 가운데 결론 카드만 넣었고, 보여 줄 상대를 보안 지식이 어느 정도 있는 사람으로 정했다 | `STATUS.md` 09-24, 이 문서 네 번째 관찰 |
| 09-24 | V10 요청서를 직접 써서 "분석 먼저, 승인 뒤 구현"을 요구했고, 분석에서 나온 수정 7개와 첫 화면 실험실 카드를 한 단계 낮추는 안을 승인했다 | `CLAUDE-V10-IMPROVEMENT-PROMPT.md`, `STATUS.md` 09-24 오후 |
| 09-25 | 독립 검토의 두 후보 가운데 결론이 증거보다 강하게 말하는 오류(후보 1)만 먼저 고쳐 배포했다. 모바일 단추 이동(후보 2)은 동기가 휴대폰에서 도움 없이 질문까지 간 것을 보고 하지 않기로 했다 | `STATUS.md` 09-25, 이 문서 여섯 번째 관찰 |

한 줄 제출안(초안): **앱이 제품을 판정하지 않고 빠진 시험 조건을 업체 질문으로 바꾸는 방향을 정했고, 처음 보는
사람에게 직접 보여 준 반응을 근거로 무엇을 고칠지(첫 화면 설명, 결론 카드)와 무엇을 하지 않을지(캐릭터, 모바일
단추 이동)를 골랐다.**
...
구현 단계(V9 이후)에서 AI 제안을 넣지 않은 기록은 아래와 같다.

| AI 제안 | 결과 | 기록 |
|---|---|---|
| Claude: 비전공 지인의 "그래서 뭐?" 반응에 세 가지(첫 화면 질문형 제목, 결과 결론 카드, 99.88% 예시로 시작)를 제안 | 사용자가 **결론 카드만** 넣기로 했다(2026-09-24). 99.88% 예시로 시작하기는 뒤에 사용자가 직접 쓴 V10 요청서에 따라 V10에서 넣었다 | 이 문서 네 번째 관찰, `DESIGN.md` 31절·32절 |
| Claude: 첫 화면을 전문 용어로 되돌리는 전문가용 첫 화면 | 제안만 했고 사용자가 정하지 않아 넣지 않았다 | `DESIGN.md` 31절, `STATUS.md` 2026-09-24 |
| Codex(V10 독립 검토): 모바일에서 `업체에 물을 질문 보기`를 결론 제목 가까이 옮기는 후보 2 | 동기가 휴대폰에서 도움 없이 질문까지 간 관찰을 근거로 하지 않았다. 검토 문서 스스로도 "관찰에서 문제가 보일 때만"을 조건으로 두었다 | `CODEX-V10-INDEPENDENT-REVIEW-HANDOFF.md` 5절, 이 문서 여섯 번째 관찰 |

이 가운데 무엇을 ③으로 쓸지도 사용자가 정한다.
