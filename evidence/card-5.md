# 카드 5 증거 — 포트폴리오와 제출

## README 포트폴리오판 제출본 — 2026-09-25

> 이 섹션이 아래 V10 결론 수정판(`3ae1312`)의 실행 묶음 값을 대체한다. 앱 코드는 `3ae1312`와 같고 README만 바뀌었다.

| 항목 | README 포트폴리오판 제출본 |
|---|---|
| 기준 커밋 | `f34f7d5` (앱 코드 `3ae1312` + README 포트폴리오판 `dfbbc61`·`f34f7d5`) |
| 배포 | GitHub Actions [run 36029761981](https://github.com/myeongjundev/explainsoc/actions/runs/36029761981) 검사·Pages 성공 |
| 공개 주소 | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-f34f7d5.zip` · 198,111 bytes · 71개 파일 |
| SHA-256 | `9b30bbd2ca626065cb025262fd92775c93b227b619ddfe158b23a724f4903e57` |
| 재현 | 같은 커밋으로 두 번 생성한 SHA-256 일치 |
| 빈 폴더 검증 | 저장소 밖 새 임시 폴더에서 `npm ci`(취약점 0), Vitest 152개, 프로덕션 빌드, 안전 검사 11개, Playwright 35개 통과(증거 캡처 26개 건너뜀) |
| 공개본 검증 | JS `index-DfnmrLxk.js`, CSS `index-DhJb78PJ.css` 로컬 빌드와 SHA-256 일치(`3ae1312`와 같은 자산) |
| README | 저장소 첫 화면에서 배지 5개와 화면 6장이 모두 불러와짐. 저장소에만 있는 화면·문서는 ZIP 안 README에서도 열리도록 절대 주소 |
| 12번 카드 | `about` 커밋 `fbf5948` — 요약과 사례 노트(문제·판단·구현·검증)를 README 30초 요약에 맞춤. 그림 `docs/assets/t13-conclusion-v9.png`·링크·152개·35개는 그대로 |

## V10 결론 수정판 제출본 — 2026-09-25

> 이 섹션은 위 README 포트폴리오판으로 대체됐다. 아래 V10 제출본(`5690d62`)의 실행 묶음과 검증 값을 대체했던 기록이다.

| 항목 | V10 결론 수정판 제출본 |
|---|---|
| 기준 커밋 | `3ae1312e103b3fd494594b9c8f00d05d4aa07eb5` (독립 검토 수정 후보 1) |
| 배포 | GitHub Actions [run 36021634394](https://github.com/myeongjundev/explainsoc/actions/runs/36021634394) 검사·Pages 성공 |
| 공개 주소 | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-3ae1312.zip` · 196,359 bytes · 71개 파일 |
| SHA-256 | `e0429ac2e1696c3fd95b791e11a279ed776bdf0feabe46e8d9f1d0bb6c511ac6` |
| 재현 | 같은 커밋으로 두 번 생성한 SHA-256 일치 |
| 빈 폴더 검증 | 저장소 밖 새 임시 폴더에서 `npm ci`(취약점 0), Vitest 152개, 프로덕션 빌드, 안전 검사 11개, Playwright 35개 통과(증거 캡처 26개 건너뜀) |
| 공개본 검증 | JS `index-DfnmrLxk.js`, CSS `index-DhJb78PJ.css` 로컬 빌드와 SHA-256 일치 · 공개 주소에서 브라우저 검사 35개 통과 |
| 12번 카드 | `about` 커밋 `b4084d4` — 그림 `docs/assets/t13-conclusion-v9.png` 그대로, 검증 문장 152개·35개 |

화면 구성은 V10과 같다. 바뀐 것은 결론 한 갈래다. "시험에 학습 때 없던 공격이 들어 있었다"는 답만 있고 분할 방식이
"따로 시험"이 아니면, 결론이 더는 "처음 보는 공격으로 시험한 숫자가 있습니다"라고 말하지 않는다. 대신 그 공격만의
성능을 따로 확인한 숫자인지는 알 수 없다고 말한다. 근거는 `planning/CODEX-V10-INDEPENDENT-REVIEW-HANDOFF.md` 4절이다.

## V10 품질 개선판 제출본 — 2026-09-24

> 이 섹션은 위 V10 결론 수정판 제출본으로 대체됐다. 아래 V9 제출본의 실행 묶음과 검증 값을 대체했던 기록이다.

| 항목 | V10 제출본 |
|---|---|
| 기준 커밋 | `5690d621721909abcad7de92fd75de0234943e52` (V10 구현 `8a72fe2` + README·증거 캡처) |
| 배포 | GitHub Actions [run 35969412385](https://github.com/myeongjundev/explainsoc/actions/runs/35969412385) 검사·Pages 성공 |
| 공개 주소 | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-5690d62.zip` · 195,103 bytes · 71개 파일 |
| SHA-256 | `bec1367329ca9fd3f8249d687c225075e6b95492bae590b28931e698c6b9d5ea` |
| 재현 | 같은 커밋으로 두 번 생성한 SHA-256 일치 |
| 빈 폴더 검증 | 저장소 밖 새 임시 폴더에서 `npm ci`(취약점 0), Vitest 146개, 프로덕션 빌드, 안전 검사 11개, Playwright 35개 통과(증거 캡처 26개 건너뜀) |
| 공개본 검증 | JS `index-EYftpwNU.js`, CSS `index-DhJb78PJ.css` 로컬 빌드와 SHA-256 일치 · 공개 주소에서 브라우저 검사 35개 통과 |
| 12번 카드 | `about` 커밋 `029b56c` — 그림 `docs/assets/t13-conclusion-v9.png` 그대로, 검증 문장 146개·35개 |

V10은 기능을 늘리지 않고 이해 순서를 다듬었다. 노트북·휴대폰 첫 화면에 주 단추가 보이게 하고, `예시로 바로 보기`가
첫 화면의 99.88% 이야기와 같은 예시를 열게 했다. 결론 카드 아래 요약은 한 줄로 줄였고, 질문마다 왜 묻는지를 붙였다.
설계 근거는 `planning/DESIGN.md` 32절이다. V10 증거 화면은 `evidence/v10-first-screen-laptop.png`,
`evidence/v10-first-screen-phone.png`, `evidence/v10-result-top-laptop.png`, `evidence/v10-questions-why.png`에 있다.

## V9 소개서 판독·결론 카드·논문 실험실 제출본 — 2026-09-24

> 이 섹션은 위 V10 제출본으로 대체됐다. 아래 V8 제출본의 실행 묶음과 검증 값을 대체했던 기록이다.

| 항목 | V9 제출본 |
|---|---|
| 기준 커밋 | `d14171e1cbe1493762d1476fabfdaea0ba298b68` (V9 구현 + 디자인 + README·증거 캡처) |
| 배포 | GitHub Actions [run 35947858700](https://github.com/myeongjundev/explainsoc/actions/runs/35947858700) 검사·Pages 성공 |
| 공개 주소 | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-d14171e.zip` · 190,431 bytes · 71개 파일 |
| SHA-256 | `3ce9908d2d26d4d2a2c593f01903b8f07c69c8f8be8fccc0b14390b804479299` |
| 재현 | 같은 커밋으로 두 번 생성한 SHA-256 일치 |
| 빈 폴더 검증 | 저장소 밖 새 임시 폴더에서 `npm ci`(취약점 0), Vitest 141개, 프로덕션 빌드, 안전 검사 11개, Playwright 31개 통과(증거 캡처 22개 건너뜀) |
| 공개본 검증 | JS `index-CFObKeM9.js`, CSS `index-D8QQMwkU.css` 로컬 빌드와 SHA-256 일치 · 공개 주소에서 브라우저 검사 31개 통과 |
| 12번 카드 | `about` 커밋 `0f11a11` — 그림 `docs/assets/t13-conclusion-v9.png` 768×705, 검증 문장 141개·31개 |

V9는 받은 소개서 문장을 그대로 붙여 넣어 그 위에서 판독하고(규칙, AI 호출 없음), 결과 맨 위에 결론 카드
(결론 · 왜? · 그래서?)를 두고, 앱이 쓰지 않던 논문 뒤쪽 절반(원고 V-2~V-5)을 논문 실험실의 실험 네 개로
직접 바꿔 보게 했다. 판독 규칙 R15(설명 가능한 AI 주장)를 더했다. 설계 근거는 `planning/DESIGN.md` 31절이다.

V9 증거 화면은 `evidence/v9-brochure-reader-desktop.png`, `evidence/v9-conclusion-desktop.png`,
`evidence/v9-conclusion-mobile.png`, `evidence/v9-paper-lab-desktop.png`, `evidence/v9-paper-lab-mobile.png`에 있다.
`evidence/screenshots/`의 a·f 장면도 V9 화면으로 다시 찍었다.

## V8 밝은 문서형 화면 제출본 — 2026-09-23

> 이 섹션은 위 V9 제출본으로 대체됐다. 아래 V7 제출본의 실행 묶음과 검증 값을 대체했던 기록이다.

| 항목 | V8 제출본 |
|---|---|
| 구현 커밋 | `169d89e79d03bb30135ece9ab3a52e253bc0fc4e` (화면 개편 `4bb1a7a` + 근거 패널 통합) |
| 배포 | GitHub Actions [run 35859205922](https://github.com/myeongjundev/explainsoc/actions/runs/35859205922) 검사·Pages 성공 |
| 공개 주소 | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-169d89e.zip` · 151,986 bytes · 64개 파일 |
| SHA-256 | `87e956cee7e05486c69d28e56b3bcf4e610e41f5caee254ba47baddab15acb6d` |
| 재현 | 같은 커밋으로 두 번 생성한 SHA-256 일치 |
| 빈 폴더 검증 | 저장소 밖 새 임시 폴더에서 `npm ci`(취약점 0), Vitest 99개, 프로덕션 빌드, 안전 검사 11개, Playwright 27개 통과(증거 캡처 17개 건너뜀) |
| 공개본 검증 | JS `index-q6bbu4Ws.js`, CSS `index-DC9q-JfQ.css` 로컬 빌드와 SHA-256 일치 · 공개 주소에서 브라우저 검사 27개 통과 |
| 12번 카드 | `about` 커밋 `bbc9424` — 그림 `docs/assets/t13-review-summary-v8.png` 343×483, 검증 문장 99개·27개 |

V8은 기능을 늘리지 않고 화면만 바꿨다. 첫 화면을 포함해 밝은 문서형으로 바꾸고 영어 라벨을 한국어로
옮겼다. 처음 입력은 질문 여섯 개를 한 화면에 하나씩 묻고, 결과에서 고치거나 다음 회차를 적을 때는 한
페이지에서 고친다. 결과 1장에 두 번 나오던 근거 목록은 근거 확인표 하나로 합쳤다. 설계 근거는
`planning/DESIGN.md` 30절이다.

V8 증거 화면은 `evidence/v8-guided-question-desktop.png`, `evidence/v8-guided-question-mobile.png`,
`evidence/v8-one-page-form-desktop.png`, `evidence/v8-chapter-evidence-desktop.png`,
`evidence/v8-chapter-evidence-mobile.png`에 있다. `evidence/screenshots/`의 a·b·c·f 장면도 V8 화면으로 다시 찍었다.

## V7 결과 밀집 해소 제출본 — 2026-09-23

> 이 섹션이 아래 V1~V6 실행 묶음과 검증 값을 대체한다. 관찰 기록은 BRB-C11 절에 그대로 남긴다.

| 항목 | V7 제출본 |
|---|---|
| 구현 커밋 | `4489803e2a4d5b79f64c1cec235ed4d5f34baa65` |
| 배포 | GitHub Actions `Verify and deploy to GitHub Pages` 검사·Pages 성공 |
| 공개 주소 | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-4489803.zip` · 145,603 bytes · 64개 파일 |
| SHA-256 | `3eca59e1007b9767ef5bd4f0953f098feb048ced5fc34be4aa5623e0536c53dc` |
| 재현 | 같은 커밋으로 두 번 생성한 SHA-256 일치 |
| 빈 폴더 검증 | 저장소 밖 새 임시 폴더에서 `npm ci`(취약점 0), Vitest 95개, 프로덕션 빌드, 안전 검사 11개, Playwright 26개 통과 |
| 공개본 검증 | JS `index-BkE6dFvd.js`, CSS `index-CJSslC0U.css` 로컬 빌드와 SHA-256 일치 · 공개 주소에서 브라우저 검사 26개 통과 |

V7은 앱을 처음 본 두 사람의 관찰(결과 화면이 밀집해 읽기 힘들다, 그림이 있으면 좋겠다)을 반영했다.
결과를 `주장과 근거 / 판독 / 다음 행동` 세 장으로 나눠 **한 번에 한 장만** 보이게 했고(1280px에서
5.2화면·패널 16개 → 장마다 3.5~4.6화면), 장 아래에 `이전 / 다음`과 `N / 3` 위치를 두었다. 받은
혼동행렬은 `공격 220,788건 가운데 탐지 160건 · 0.072%`, `정상 375,518건 가운데 오탐 86건 · 0.023%`를
실제 비율 막대로 그린다. 축을 자르거나 최소 너비를 주지 않아 탐지 막대가 거의 보이지 않으며, 그
사실을 눈금과 글로 알린다. 캐릭터·삽화는 넣지 않았다. 수치·논문 근거·R01~R14·JSON·보안 경계는
그대로다. 설계 근거는 `planning/DESIGN.md` 29절이다.

V7 증거 화면은 `evidence/v7-chapter-evidence-desktop.png`, `evidence/v7-chapter-evidence-mobile.png`,
`evidence/v7-scale-figure-desktop.png`에 있다.

## V6 비전공자 첫 화면 명료화 제출본 — 2026-09-22

> 이 섹션이 아래 V1~V5 실행 묶음과 검증 값을 대체한다. 첫 관찰의 실패 기록은 BRB-C11 절에 그대로 남긴다.

| 항목 | V6 제출본 |
|---|---|
| 구현 커밋 | `b5384eee193add96acbacd1f59d2dbf5d06a9577` |
| 배포 | [GitHub Actions run 35732418454](https://github.com/myeongjundev/explainsoc/actions/runs/35732418454) 검사·Pages 성공 |
| 공개 주소 | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-b5384ee.zip` · 140,962 bytes · 63개 파일 |
| SHA-256 | `5cda7ec171d36a352dd3cd33f5b17a1092453a8356288862d4200bd189b8c87b` |
| 빈 폴더 검증 | 저장소 밖 새 임시 폴더에서 `npm ci`, 안전 검사 11개, Vitest 92개, 프로덕션 빌드, 미리보기 HTTP 200, Playwright 26개 통과(증거 캡처 9개 제외) |
| 공개본 검증 | JS `index-CRXUGNHw.js`, CSS `index-BHoV4po4.css` 로컬 빌드와 SHA-256 일치 · 기능·품질 26개와 증거 캡처 9개 통과 |

V6는 비전공자 첫 관찰에서 사이트 목적이 전달되지 않은 문제를 숨기지 않고 고친 결과다. 제목에 `보안 AI`를
직접 쓰고, `광고 숫자가 무엇을 시험한 결과인지 확인해 판매 업체에 물을 질문을 만드는 도구`라고 설명한다.
회사에서 AI 제품 자료를 받았지만 어떤 공격을 시험했는지 모르는 상황을 한 줄로 보이며, 세 단계는 `숫자 입력 →
모르는 시험 조건 답변 → 업체 질문 완성`으로 쓴다. 수치·논문·R01~R14·결과 구조·보안 경계는 바꾸지 않았다.

## V5 대표 앱 제출본 — 2026-09-22

> V6 전 최종 V5 기록이다. 현재 제출본은 바로 위 V6 기록을 따른다.

| 항목 | V5 제출본 |
|---|---|
| 구현 커밋 | `7d0a105f8b14dd05e635a56dce261e87ea0b6933` |
| 배포 | [GitHub Actions run 35728715004](https://github.com/myeongjundev/explainsoc/actions/runs/35728715004) 검사·Pages 성공 |
| 공개 주소 | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-7d0a105.zip` · 140,605 bytes · 63개 파일 |
| SHA-256 | `1b2854d4a847a5606711b4ba020adc6595ca698f5aefc96a0bdc668f7c21fe92` |
| 빈 폴더 검증 | 저장소 밖 새 임시 폴더에서 `npm ci`, 안전 검사 11개, Vitest 92개, 프로덕션 빌드, Playwright 26개 통과(증거 캡처 9개 제외) |
| 공개본 검증 | JS `index-WXO94w-9.js`, CSS `index-i0wfD4Ar.css` 로컬 빌드와 SHA-256 일치 · 기능·품질 26개와 증거 캡처 9개 통과 |

V5는 기능을 더 늘리지 않고 결과를 읽는 순서를 고쳤다. 결과 첫 화면은 `Decision Brief`에서 현재 회차,
확인 필요·해석 주의·다음 질문 수와 첫 질문을 먼저 보여 준다. 그 아래에서 주장과 근거·판독·다음 행동을
독립된 세 열로 읽고, 회차 기록은 필요할 때만 펼친다. 모바일은 세 장을 한 번에 길게 쌓지 않고 한 장씩
전환한다. `판정 아님 · 근거 확인` 원칙, R01~R14, JSON, 개인정보와 CSP 경계는 그대로 유지했다.

V5 전용 증거 화면은 `evidence/v5-investigation-board-desktop.png`,
`evidence/v5-separate-branch-desktop.png`, `evidence/v5-briefing-mobile.png`에 있다. 공개 주소에서도 같은
세 장을 `evidence/screenshots/live/`에 다시 찍었다.

## V4 대표 앱 제출본 — 2026-09-22

> V5 전 최종 V4 기록이다. 현재 제출본은 문서 맨 위 V6 기록을 따른다.

| 항목 | V4 제출본 |
|---|---|
| 구현 커밋 | `32208672ed96d87fcd1ccb319d2c66a6debce41c` |
| 배포 | [GitHub Actions run 35700989600](https://github.com/myeongjundev/explainsoc/actions/runs/35700989600) 검사·Pages 성공 |
| 공개 주소 | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-3220867.zip` · 136,265 bytes · 62개 파일 |
| SHA-256 | `8f0ef71046c9bd643fa1d18e3c1bb3c37ca57ca6b1657b1b2193344c75128a1e` |
| 재현 | 같은 커밋으로 두 번 생성한 SHA-256 일치 |
| 빈 폴더 검증 | 저장소 밖 새 임시 폴더에서 `npm ci`, 프로덕션 빌드, 안전 검사 11개, Vitest 91개, Playwright 25개 통과(증거 캡처 9개 제외) |
| 공개본 검증 | JS `index-CfDxTmKS.js`, CSS `index-2ZwK-MBR.css` 로컬 빌드와 SHA-256 일치 · Playwright 25개 통과 |

### 집 PC 제출 직전 재검증 — 2026-09-22

학원에서 올린 `8aa382a`까지 fast-forward한 뒤 집 PC의 Node.js 24.15.0, npm 11.12.1에서 다시
확인했다. 구현 파일은 바꾸지 않았다.

| 확인 | 결과 |
|---|---|
| 고정 설치 | `npm ci` · 113개 패키지 설치 · 취약점 0개 |
| 전체 검증 | 안전 검사 11개 · Vitest 91개 · 프로덕션 빌드 · Playwright 25개 통과(증거 캡처 9개 제외) |
| 공개 주소 | 로그인·쿠키·저장소가 없는 새 브라우저 컨텍스트에서 Playwright 25개 통과 |
| V4 ZIP 재생성 | `explainsoc-3220867.zip` · 136,265 bytes · 파일 62개 |
| SHA-256 대조 | `8f0ef71046c9bd643fa1d18e3c1bb3c37ca57ca6b1657b1b2193344c75128a1e` · 학원 PC 기록과 일치 |

V4는 기존 R01~R14·회차 JSON·보안 경계를 유지하면서 화면의 제품 서사를 바꿨다. 첫 화면에서
`99.88% → 시험 조건 → 220,788건 중 160건 탐지`를 해부하고, 결과는 `주장과 근거 / 판독 / 다음 행동`
3열 수사 보드로 읽는다. 같은 시험·미확인·별도 시험은 회차 타임라인에서 다른 분기로 보이며,
Decision Dossier 앞장은 다음 회의의 질문 수와 우선 질문을 원문 기록보다 먼저 보여 준다.

V4 전용 증거 화면은 `evidence/v4-investigation-board-desktop.png`,
`evidence/v4-separate-branch-desktop.png`, `evidence/v4-dossier-mobile.png`에 있다. 공개 주소에서도 같은
세 장을 `evidence/screenshots/live/`에 다시 찍었다.

## V3 제출본 갱신 — 2026-09-22

> V4 전 최종 V3 기록이다. 현재 제출본은 문서 맨 위 V6 기록을 따른다.

| 항목 | V3 제출본 |
|---|---|
| 구현 커밋 | `679add3c250b36545b319a80f9f55c847e56b1ba` |
| 배포 | [GitHub Actions run 35698164859](https://github.com/myeongjundev/explainsoc/actions/runs/35698164859) 검사·Pages 성공 |
| 공개 주소 | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-679add3.zip` · 130,786 bytes · 61개 파일 |
| SHA-256 | `80650a6729c912dadd071adb87384d59e253660a82d4296cefa0a0503ecffdbd` |
| 재현 | 같은 커밋으로 두 번 생성한 SHA-256 일치 |
| 빈 폴더 검증 | 저장소 밖 새 임시 폴더에서 `npm ci`, 프로덕션 빌드, 안전 검사 11개, Vitest 91개, Playwright 24개 통과(증거 캡처 9개 제외) |
| 공개본 검증 | JS `index--yn4oNyf.js`, CSS `index-D1Vo0e1F.css` 로컬 빌드와 SHA-256 일치 · Playwright 24개 통과 |

V3는 최초 주장과 후속 공급자 답변을 회차로 비교하고, 같은 시험 여부 R14, JSON 저장·열기,
공급자 자료 요청서, 회차 요약 검토표를 포함한다. 당시 제출할 ZIP은 V1·V2 묶음이 아니라
**`explainsoc-679add3.zip`**이다. 독립 리뷰 R1~R5와 작은 결함 L1~L3까지 반영해 별도 시험을
해결로 표시하지 않고, 회차별 답변 메모와 사람이 읽는 근거 풀이를 보존한다.

> 통과 기준: BRB-C10 · BRB-C11 · BRB-C12 · BRB-C13 · BRB-C14 · BRB-C15 · BRB-C22
> 기록일: 2026-09-22 · 기록한 사람: Claude(구현 담당)
>
> **사용자가 채울 곳이 세 군데 남아 있다.** 처음 보는 동료의 사용 결과와 이름 없는 한 줄(BRB-C11),
> 사람이 잰 60초, 제출문의 `② 내가 직접 판단한 일`(BRB-C14). Claude는 이 셋을 지어내지 않고 비워 둔다.
> 2026-09-25: BRB-C11은 여섯 번째 관찰로 채워졌다. 남은 것은 사람이 잰 60초와 BRB-C14 ②다.

## 12번 사이트 대표작 자리 — BRB-C10

**https://myeongjundev.github.io/about/#work-t13-app**

| 항목 | 값 |
|---|---|
| 저장소·커밋 | `myeongjundev/about` [`64945b4`](https://github.com/myeongjundev/about/commit/64945b4) (V6 쉬운 한 문장) — V5 그림·검증 수 `e53e2c3` |
| 바꾼 것 | `content/approved.json`의 `t13-app`을 `planned` → `published`. 예정일을 빼고 이름·한 문장·사례 노트 넷·그림·링크 셋을 채웠다 |
| 이름 | ExplainSOC — 성능표에 없는 질문 |
| 한 문장 | ‘정확도 99%’ 같은 보안 AI 광고 숫자가 실제로 무엇을 시험한 결과인지 확인하고, 판매 업체에 물어볼 질문을 10번 논문 근거와 함께 만드는 앱입니다. |
| 링크 | 앱 체험 (공개 주소) · 검증 기록 (`evidence/card-4.md`) · 공개 저장소 |
| 그림 | `docs/assets/t13-decision-brief-v5.png` 343×498 — 공개 주소의 Decision Brief 모바일 장면 |

한 문장은 앱 첫 화면의 도움 문장에 "10번 논문"만 더했다. 12번 사이트에서 바로 위 카드가 10번
논문이라 둘이 이어져 읽힌다.

사례 노트 넷은 이 저장소의 증거에 있는 사실만 썼다.

| 칸 | 문장 | 근거 |
|---|---|---|
| 문제 | 10번 논문에서 같은 XGBoost가 무작위 분할에서는 Macro F1 0.9979, 학습에 없던 공격을 시험하면 0.3871이었습니다. 제품 소개서의 성능 숫자 하나로는 무엇을 시험한 점수인지 알 수 없습니다. | 원고 V-2, `evidence/card-1.md` |
| 판단 | 숫자의 좋고 나쁨을 판정하는 대신, 먼저 읽어야 할 해석 주의와 다음 질문을 Decision Brief로 요약하고 근거·판독·다음 행동을 단계별로 확인하게 했습니다. | `planning/DESIGN.md` 27절 |
| 구현 | React·TypeScript·Vite 정적 앱에 Claim Autopsy, Decision Brief, 모바일 장면 전환, 6항목 근거 지도, 시험 분기 타임라인, 회차 JSON과 Decision Dossier를 연결했습니다. 입력은 브라우저 안에서만 다루고 콘텐츠 보안 정책으로 외부 전송을 막았습니다. | V5 구현 커밋 `7d0a105` |
| 검증 | 단위·컴포넌트 테스트 92개와 WCAG 2.2 AA·375px·키보드·인쇄를 포함한 실제 브라우저 테스트 26개를 통과한 커밋만 배포되게 했습니다. 재현 가능한 ZIP을 저장소 밖 빈 폴더에 풀어 설치·빌드·브라우저 검사까지 다시 통과했습니다. | 이 문서 V5 제출본 기록 |

처음 연결할 때는 74개였고 V1 카드에서 75개로 고쳤다(`ea77b4e`). 현재 수치는 92개·26개다.

**V5 그림을 Decision Brief로 바꾼 이유.** 포트폴리오 카드 한 장에서도 앱의 원칙과 사용자의 다음 행동이
바로 읽혀야 한다. 전체 보드를 축소하지 않고 `판정 아님 · 근거 확인`, 해석 주의 수, 첫 질문이 한 장에
보이는 모바일 브리핑을 사용했다.

| V5 대표 카드 그림 |
|---|
| ![12번 사이트 ExplainSOC V5 Decision Brief](screenshots/t12-t13-decision-brief-v5.png) |

공개 사이트의 카드 상태는 `LIVE`, 종류 줄은 `CASE 04 · 13번 과제 · 앱 · 2026.09`다. 공개 HTML이 이
그림을 사용하고 92개·26개 검증 문장을 담는지 확인했으며, 공개 이미지 SHA-256도 로컬과 같다. 이전
데스크톱·모바일·왼쪽 카드 화면은 V4 갱신 당시 증거로 `evidence/screenshots/t12-*.png`에 보존했다.

### 12번 저장소에서 확인한 것

| 검사 | 결과 |
|---|---|
| `python -m unittest device/test_device.py` | 20개 OK |
| `python device/check_repeat.py` | PASS — 반복 결과와 기대 결과가 같다 |
| `python device/validate_release.py --check-urls` | PASS — 새 링크 셋 포함 외부 링크 전부 |
| `python release/build_release.py` | PASS — 로컬 `release/out/T12-KimMyeongjun.zip` 재생성 |
| GitHub Actions | [T12 checks run 35732677557](https://github.com/myeongjundev/about/actions/runs/35732677557) 성공 |
| GitHub Pages | [pages run 35732676543](https://github.com/myeongjundev/about/actions/runs/35732676543) 성공 · 공개 HTML에 쉬운 한 문장과 V5 그림·26개 검증 문장 확인 |

- 12번 테스트에 사례 노트 수가 3·12로 적혀 있어 4·16으로 고쳤다. 13번 카드가 왼쪽 카드와 대표작에
  모두 있는지 보는 줄 둘을 더했다.
- 왼쪽 카드가 7장에서 8장이 됐다. 겹침 간격 검사(마지막 카드 top + 330px ≤ 560px)는 8장까지
  들어간다. 카드 강조색은 6장까지만 있어 7·8번째는 기본 파랑이다. 7번째는 이전부터 그랬다. 12번의
  디자인이라 손대지 않았다.
- 문서 3종(이력서·자기소개서·경력기술서)은 대표작을 읽지 않는다. 다시 만들어 본문(`word/document.xml`)이
  같음을 확인하고, 압축 시각만 다른 DOCX는 원래 파일로 되돌렸다. 13번을 이력서에 넣을지는 사용자가 정한다.

## 처음 보는 동료 한 명 — BRB-C11

### 첫 관찰 — 목적 전달 실패, 재시험 필요

2026-09-22 사용자가 이 앱을 처음 본 비전공자 한 명에게 공개 주소를 보여 줬다. 전달받은 관찰은
**첫 화면만 보고 무엇을 하는 사이트인지 이해하지 못했다**는 것이다. 비전공자라는 사실 외에 기기·
브라우저, 걸린 시간, 세 행동 완주 여부는 기록되지 않았다. 이름 없는 한 줄의 직접 인용도 받지 않아
사용자 전달 요약만 남긴다.

| 항목 | 기록 |
|---|---|
| 대상 | 이 앱을 처음 본 비전공자 1명 · 이름 기록 안 함 |
| 1분 안에 목적 설명 | 아니오로 관찰 · 시간은 재지 않음 |
| 막힌 곳 | 첫 화면에서 사이트의 용도를 이해하지 못함 |
| 사용자 전달 요약 | “뭐 하는 사이트인지 잘 모르겠다”는 반응 |
| 세 행동·60초 경로 | 확인하지 못함 |

따라서 BRB-C11을 완료로 표시하지 않는다. 이 결과를 근거로 첫 화면을 쉬운 말과 구체적 사용 상황으로
고친 뒤, **기존 참가자가 아닌 새로운 처음 보는 사람**에게 다시 시험한다(`DESIGN.md` 28절).

### 두 번째 관찰 — 결과 화면이 밀집해 읽기 힘들다

2026-09-23 사용자가 V6를 두 사람에게 보여 주고 전달한 관찰이다. 기기·브라우저, 걸린 시간, 세 행동
완주 여부, 직접 인용은 받지 않았다. 따라서 이 관찰도 BRB-C11 완료 증거가 아니다.

| 항목 | 기록 |
|---|---|
| 대상 | 앱을 처음 본 2명 · 이름 기록 안 함 |
| 막힌 곳 | 버튼을 눌러 다음으로 넘어간 뒤 보이는 것들이 너무 밀집해 보기 힘들다 |
| 요청 | 캐릭터나 그래프, 보이는 이미지가 있으면 더 좋겠다 |
| 세 행동·60초 경로 | 확인하지 못함 |

Claude가 같은 화면을 직접 재어 관찰을 뒷받침했다: 1280px 결과 화면은 5.2화면 길이에 패널 16개를
세 열로 동시에 펼친다. 이 사실을 근거로 V7에서 결과를 한 번에 한 장만 보이게 바꾸고, 받은 혼동행렬을
실제 비율 막대로 그렸다(`DESIGN.md` 29절). 캐릭터는 넣지 않기로 사용자와 정했다. 도구의 성격을 바꾸고,
숫자 자체를 설명하지 못하기 때문이다.

### 세 번째 관찰 — 첫 화면이 전문적이라 쓰지 않을 것 같다

2026-09-24 사용자가 V9 작업본(`v9-brochure` 브랜치, 사용자 PC의 로컬 미리보기)을 비전공 지인 한 명에게 PC로
보여 주고 전달한 관찰이다. 반응은 첫 화면에서 나왔다. 걸린 시간, 버튼을 눌러 봤는지, 세 행동 완주 여부는
기록되지 않았다. 따라서 BRB-C11 완료 증거가 아니다.

| 항목 | 기록 |
|---|---|
| 대상 | 비전공 지인 1명 · 이름 기록 안 함 |
| 기기·화면 | PC 브라우저 · V9 작업본 첫 화면(`localhost` 미리보기) |
| 사용자가 전한 반응 | “이게 뭔데? 어떻게 하는 건데? 음 잘 모르겠는데? 뭔가 전문적인 거 같은데 난 잘 몰라서 사용하지 않을 것 같아.” |
| 막힌 곳 | 첫 화면에서 무엇을 하는 사이트인지, 어떻게 쓰는지 알지 못함 |
| 세 행동·60초 경로 | 확인하지 못함 |

첫 관찰(V6 전)과 같은 신호가 첫 화면 개편 뒤에도 다시 나왔다. 화면 모양보다 첫 화면의 말(분할, Macro F1,
미관측 공격 스트레스 테스트 같은 용어)과 버튼 세 개가 막고 있다고 본다. 이 사람은 앱의 대상(보안 담당자)이
아니지만, 포트폴리오를 보는 채용 심사자도 비전공자일 수 있어 첫 화면은 누구나 10초 안에 "무엇을 해 주는지"
알아야 한다. 이 근거로 V9에서 첫 화면을 시험 공부 비유로 다시 쓴다(`DESIGN.md` 31절).

### 네 번째 관찰 — 결과가 좋은지 나쁜지 알 수 없다, 그래서 뭐?

2026-09-24 첫 화면을 시험 공부 비유로 다시 쓴 뒤(`d62ee09`) 사용자가 비전공 지인에게 다시 보여 주고 전달한
반응이다. 어디까지 눌러 봤는지, 기기, 걸린 시간은 사용자도 확인하지 못했다(“잘 모름”). 결과 화면을 본 것은
반응 내용으로 알 수 있다. BRB-C11 완료 증거가 아니다.

| 항목 | 기록 |
|---|---|
| 대상 | 비전공 지인 · 이름 기록 안 함 |
| 본 화면 | V9 작업본 첫 화면과 결과 화면 · 어떤 입력으로 결과를 냈는지는 모름 |
| 사용자가 전한 반응 | 이게 뭐 하는 사이트인지 잘 모르겠다. 결과가 나왔는데 글만 읽어서는 좋은 건지 나쁜 건지 판단이 안 되고, 결과가 어떤 건지, 그래서 뭐? |
| 막힌 곳 | 결과 첫머리에 결론이 없다(“1개의 해석 주의를 먼저 읽어야 합니다”는 무엇을 하라는지 말하지 않는다) |

사용자가 세 가지 제안(첫 화면 질문형 제목, 결과 결론 카드, 99.88% 예시로 시작) 가운데 **결론 카드만** 넣기로
정했다. 결론 카드는 AI 제품을 좋다·나쁘다로 판정하지 않고, 광고 숫자만으로 믿을 근거가 있는지와 받은 숫자가
말하는 사실을 쉬운 말로 먼저 보인다(`DESIGN.md` 31절).

### 다섯 번째 관찰 — 보안 AI 광고를 본 적이 없다

2026-09-24 결론 카드를 넣은 뒤(`1e514e9`) 같은 날 사용자가 전한 비전공 지인의 말이다. 기기·시간·세 행동은
기록되지 않았다. BRB-C11 완료 증거가 아니다.

| 항목 | 기록 |
|---|---|
| 대상 | 비전공 지인 · 이름 기록 안 함 |
| 사용자가 전한 말 | 살면서 “보안 AI의 99점” 같은 광고를 잘 본 적이 없다 |
| 뜻 | 첫 화면의 출발 상황(보안 AI 광고)이 일반인에게는 낯설다. 이 앱의 대상은 보안 담당자라 일반인이 쓰지 않는 것은 문제가 아니지만, 첫 화면은 채용 심사자를 포함한 누구나 알아들어야 한다 |

이 앱의 상황(보안 AI 제품 자료를 받아 검토하는 일) 자체가 비전공자에게는
겪어 본 적 없는 일이라는 뜻이다. BRB-C11의 대상은 “처음 보는 동료”이므로, 공식 재시험은 앱의 대상에 가까운
사람(예: 학원 동기)에게 한다.

### 여섯 번째 관찰 — V10 공개 사이트, 혼자 잘했다

2026-09-25 사용자가 전한 결과다. 사용자가 공개 사이트를 동기에게 보여 줬고, 특별히 막히거나 헷갈려하지 않았다고
했다. 뒤이어 사용자가 양식 항목을 채워 주었고(아래 표), 처음에는 기억하지 못했던 한 줄 감상도 같은 날 전해 주었다.
처음 보는 동료 한 사람의 한 줄 감상이 이름 없이 적혔으므로 **BRB-C11을 완료로 표시한다.** 한 줄은 사용자가 전한
문장을 그대로 옮겼다.

| 항목 | 기록 |
|---|---|
| 날짜 | 2026-09-25 |
| 대상 | 동기 · 이름 기록 안 함 |
| 기기·브라우저 | 휴대폰 크롬 |
| 1분 안에 무슨 앱인지 말했나 | 예 (초 단위 시간은 기록 없음) |
| 업체 질문까지 도움 없이 갔나 | 예 |
| 막힌 곳 | 없음 |
| 사용자가 전한 말 | "별 반응 없던데", "혼자 잘하던데" |
| 이름 없는 한 줄 | "아, 보안 AI 업체가 99%라고 했을 때 그 숫자를 그냥 믿지 말고 어떤 테스트인지 확인하게 해주는 거구나" |
| 버전 | 기록 없음 (같은 날 공개 사이트는 `3ae1312`) |

이 관찰로 독립 검토 후보 2(모바일 결과에서 `업체에 물을 질문 보기`를 결론 제목 가까이 옮기기)는 **하지 않는다**.
휴대폰에서 도움 없이 업체 질문까지 갔으므로, 인계 문서 5절의 조건("신규 사용자가 자연스럽게 질문까지 도달하면
현재 배치를 유지")에 맞춰 현재 배치를 유지한다.

재시험할 때 사용자 할 일:

1. 이 앱을 본 적 없는 사람 한 명에게 **공개 주소만** 보낸다. 무엇을 하는 앱인지 말하지 않는다.
2. 옆에서 보기만 하고 돕지 않는다. 시계를 켠다.
3. 아래 네 가지를 적는다.
4. 끝나면 "이 앱을 한 줄로 말하면?" 또는 "써 보니 어땠어요?"를 묻고, 답을 **이름 없이** 그대로 적는다.
   이름·학교·연락처는 적지 않는다.

```text
날짜: 2026-__-__
기기·브라우저: (예: 휴대폰 크롬)
1분 안에 "누구를 어떻게 돕는 앱인지" 말했나: 예 / 아니오 (__초)
세 행동을 도움 없이 끝냈나: ① 숫자 적기 또는 예시 선택 __ ② 세 질문에 답하기 __ ③ 질문 복사 __
막힌 곳: (없으면 "없음". 있으면 본 그대로)
이름 없는 한 줄: "…"
```

받은 내용을 이 절에 붙이면 BRB-C11과 완료 정의의 "설명 없이 세 행동을 완주"를 지운다. 막힌 곳이 나오면
고치기 전에 설계 범위 안인지 먼저 본다.

## 사람이 잰 60초

**아직 없다.** 자동 측정(카드 2, 예시 경로 0.5초)은 사람이 읽는 시간이 아니다. 동료 시험 때 같이 재면 된다.

```text
첫 화면 → "논문 예시로 60초 검토" → "공격 기준으로 뒤집기" → "질문 복사"까지: __초
"공격 220,788건 가운데 탐지 160건"을 읽고 무슨 뜻인지 말했나: 예 / 아니오
```

## 전체 제출물의 실명·연락처·비밀값 — BRB-C12

| 제출물 | 본 방법 | 결과 |
|---|---|---|
| 공개 주소의 배포본 5개 파일(HTML·JS·CSS·아이콘·라이선스 고지) | 공개 서버에서 받아 비밀값·이메일·휴대전화·주민번호·데이터셋 저자 이름을 찾음 | 0건 |
| 실행 묶음 `explainsoc-b5384ee.zip` 63개 파일 | 저장소 밖 빈 폴더에 풀어서 빌드 후 `npm run check`(폴더 탐색) + 저자 이름·PC 경로 검색 | 11개 PASS · 0건 |
| 공개 저장소의 모든 파일(과제 기록·증거 포함) | `npm run check` + `git grep`으로 저자 이름, 이니셜+성 표기, 호칭(님·씨 등) 검색 | PASS · 0건 |
| 12번 사이트의 13번 카드 | 문장·그림·링크 확인, 12번 `validate_release` | 다른 사람 이름 없음 · PASS |
| 제출문 초안(이 문서 아래) | 직접 확인 | 다른 사람 이름 없음 |

**걸렸던 것 하나를 고쳤다.** 앱의 연구 범위 화면과 README의 데이터셋 인용에 데이터셋 논문 저자 세 명의
실명이 있었다. 학술 인용이지만 기준은 "제출물 어디에도 본인 이름 외의 다른 사람 실명이 없다"로 예외가
없다. 저자 이름을 빼고 논문을 하나로 가리키는 제목·학회·쪽·연도·DOI(`10.5220/0006639801080116`)를
남겼다. 저자를 포함한 전체 인용은 같은 화면에서 이어지는 10번 논문의 참고문헌 [1]에 있다고 적었다
(`3dd6274`). 인용에 DOI가 있고 이니셜+성 표기가 없는지 단위 테스트로 막았다.

그 테스트의 주석에 막으려는 표기의 예로 실제 저자 이름 둘을 적어 ZIP에 실명이 다시 들어갔다. ZIP을
풀어 검사하다 잡았고, 형식만 보이는 가짜 예("A. B. Surname")로 바꿨다(`2e7bd73`). 지금 저장소 전체에서
저자 이름 검색 결과는 0건이다. 남은 "이니셜+성" 한 건은 이 가짜 예다.

본인 이름은 13번 저장소와 앱 어디에도 없다. 본인 이름은 12번 사이트에만 있다(허용).

## 짧은 확인 방법 — BRB-C13

제출문에 그대로 옮길 수 있게 적었다. 2026-09-25 공개 주소를 쿠키·로그인 없는 새 브라우저 컨텍스트로 열어
아래 문구와 값을 그대로 확인했다(V7 때의 옛 확인 방법은 Git 기록에 있다).

```text
결과물: https://myeongjundev.github.io/explainsoc/ (로그인 없음)
실행 묶음: explainsoc-f34f7d5.zip — 압축을 풀고 npm ci → npm run build → npm run preview 후
  http://127.0.0.1:4173/explainsoc/ (Node.js 24.15 이상, 22.x는 22.22.2 이상)
12번 사이트 대표작: https://myeongjundev.github.io/about/#work-t13-app

어디에 무엇이
- 첫 화면: 제목 "보안 AI의 99점, 처음 보는 공격에서도 99점일까요?", 누구를 어떻게 돕는지 한 문장
  ("이 사이트가 하는 일. 회사에서 침입 탐지 같은 보안 AI 제품의 소개서를 받았을 때 …"),
  아래쪽 "반영한 논문 · 데이터 분할 방식에 따른 XAI 기반 네트워크 침입 탐지 성능과 설명 안정성 분석"
- 묶음 안: src/data/paperEvidence.ts(논문 근거 P01~P14와 수치), src/domain/reviewRules.ts(판독 규칙
  R01~R15), tests/(단위·컴포넌트 152개, 실제 브라우저 35개)

세 행동 해 보기
1) "예시로 바로 보기"를 누른다. 내 숫자로 하려면 "숫자로 직접 입력"에서 지표 하나(예: 정확도 0.99)를 적는다.
2) 시험 조건 세 질문(나눈 방식, 학습 때 없던 공격, 중복 제거)에 예 / 아니오 / 모름으로 답한다. 모르면 모름.
   (예시는 논문 값으로 이미 채워져 있다)
3) 결과의 "업체에 물을 질문 보기"를 눌러 질문과 "왜 묻나요"를 읽고 "질문만 복사"를 누른다.

무엇이 나오면 통과
- 예시: 결론 "광고 숫자만으로는 아직 믿기 이릅니다", 이유에 "99.8점이 38.7점이 됐습니다",
  "판매 업체에 아래 질문 2개를 하세요", 질문 "학습에 없던 공격만 따로 둔 시험 결과도 있습니까?"
- 결과의 "다른 예시"를 누르면 "공격 220,788건 중 160건을 잡았습니다"
- 정확도 0.99만 적고 조건은 모두 모름: 확인 필요 3 · 해석 주의 1 · 다음 질문 4
- 99를 적으면 멈추지 않고 "비율은 0부터 1 사이로 적어 주세요. 99%라면 0.99로 적습니다"
- 입력은 서버로 가지 않는다(콘텐츠 보안 정책 connect-src 'none'). 새로 고치면 사라진다.
```

## 제출 목록 — BRB-C22 · BRB-C15

| 무엇 | 값 |
|---|---|
| 결과물 URL | https://myeongjundev.github.io/explainsoc/ |
| 실행 묶음 | `explainsoc-f34f7d5.zip` · 198,111 bytes · 71개 파일 · SHA-256 `9b30bbd2ca626065cb025262fd92775c93b227b619ddfe158b23a724f4903e57` · 원본 커밋 `f34f7d5` · 로컬 저장소 `release/` |
| 12번 사이트 연결 | https://myeongjundev.github.io/about/#work-t13-app — 앱 체험·검증 기록·공개 저장소 링크 |
| 라이선스 | MIT (`LICENSE`, 2026-09-22 사용자 결정). GitHub도 저장소를 MIT로 인식한다 |
| 짧은 확인 방법 | 위 BRB-C13 |
| 제출문 세 줄 | 아래 BRB-C14 |

현재 제출물은 README 포트폴리오판 커밋으로 만든 **`explainsoc-f34f7d5.zip`**이다(앱 코드는 `3ae1312`와 같다).
저장소 밖 새 임시 폴더에서 고정 설치부터 전체 검사를 다시 통과했다(맨 위 표). 다시 만드는 방법은
`npm run release:zip -- f34f7d5`이다.

### 제출할 수 있는 URL — 모두 로그인 없이

| URL | 응답 |
|---|---|
| https://myeongjundev.github.io/explainsoc/ | 200 · 리디렉션 0 · 인증 헤더 0 |
| https://github.com/myeongjundev/explainsoc | 200 · 0 · 0 |
| https://myeongjundev.github.io/about/ (13번 카드 `#work-t13-app`) | 200 · 0 · 0 |
| https://github.com/myeongjundev/explainsoc/blob/main/evidence/card-4.md | 200 · 0 · 0 |
| https://github.com/myeongjundev/explainsoc-research | 200 · 0 · 0 |
| https://github.com/myeongjundev/explainsoc-research/blob/main/output/pdf/T10-research-paper.pdf | 200 · 0 · 0 |
| https://myeongjundev.github.io/explainsoc/third-party-licenses.md | 200 · 0 · 0 |

2026-09-25 제출본(`3ae1312`) 기준으로 위 7개를 다시 열어 모두 200 · 리디렉션 0을 확인했다. 아래 문단은 V6 당시 기록이다.

공개 주소는 새 브라우저 컨텍스트(쿠키·저장소·로그인 없음)에서 기능·품질 테스트 26개와 증거 캡처
9개를 다시 통과했다. 최종 배포본 `index-CRXUGNHw.js`와 `index-BHoV4po4.css`는 로컬 빌드와
SHA-256이 같다.

### 최종 묶음의 새 임시 폴더 실행

저장소 밖 세션 임시 폴더 아래 빈 폴더에 PowerShell `Expand-Archive`로 풀었다. 아래는 V6 묶음의 결과다.

```text
<새 임시 폴더>\explainsoc-b5384ee> npm ci
added 113 packages, and audited 114 packages
found 0 vulnerabilities

<새 임시 폴더>\explainsoc-b5384ee> npm run build
✓ 50 modules transformed.
dist/assets/index-BHoV4po4.css
dist/assets/index-CRXUGNHw.js

<새 임시 폴더>\explainsoc-b5384ee> npm run preview
  ➜  Local:   http://127.0.0.1:4173/explainsoc/
```

| 확인 | 결과 |
|---|---|
| 풀린 파일 | 63개(`LICENSE` 포함), 처음 풀었을 때 `node_modules`·`.git`·`dist` 없음 |
| 자산 이름 | 공개 주소와 같다(`index-CRXUGNHw.js`, `index-BHoV4po4.css`) |
| `npm run check` | 빌드 후 폴더 탐색 · 63개 파일 · 앱 소스 38개 · 11개 PASS |
| `npm test` | 92개 통과 |
| `npm run test:e2e` | 26개 통과, 증거 캡처 9개 제외 |

## 제출문 세 줄 — BRB-C14

원문은 세 가지를 나눠 적으라고 한다. ①과 ③은 기록에 있는 사실만 초안으로 두었고, ②는 2026-09-25 사용자 요청으로
기록된 사용자 결정에서 초안을 만들어 다듬었다. **제출할 세 줄은 맨 아래 "제출 준비"에 모았다.**

### ① AI에게 맡긴 일 — 사실 초안

Codex와 Claude에게 과제·논문 분석, 제품 대안 검토와 설계, React·TypeScript 구현, 판독 규칙
R01~R15, 접근성·보안 검증, GitHub Pages 배포, 재현 가능한 ZIP과 카드별 증거 기록을 맡겼다. V10 뒤
독립 검토도 Codex에게 맡겼다. 현재 제출본(`f34f7d5`)은 단위·컴포넌트 152개와 실제 브라우저 35개를 통과했다.

한 줄 제출안: **과제·논문 분석부터 제품 설계, React 구현, 판독 규칙 R01~R15와 접근성·보안 검증,
Pages 배포와 재현 가능한 ZIP·증거 기록까지 Codex와 Claude에게 맡겼다.**

### ② 내가 직접 판단한 일

> 2026-09-25 사용자 요청으로 Claude가 `planning/STATUS.md`와 이 문서에 남은 **사용자 결정만** 모아 초안을 만들고,
> 다시 사용자 요청으로 1인칭 글로 다듬었다. 기록에 없는 판단은 넣지 않았다. 제출 전 사용자가 자기 기억과 맞는지 본다.

다듬은 글:

> 앱이 AI 제품을 좋다·나쁘다로 판정하지 않고, 광고 숫자에서 빠진 시험 조건을 판매 업체에 물을 질문으로
> 바꾸게 방향을 정했습니다. 그다음에는 기능을 늘리기보다 처음 보는 사람에게 직접 보여 주고 반응을 보며
> 무엇을 고칠지 골랐습니다. "무슨 사이트인지 모르겠다"는 말에 첫 화면을 쉬운 말로 바꿨고, "그래서 뭐?"라는
> 말에는 AI가 낸 세 가지 안 가운데 그때는 결과 맨 위 결론 카드만 골라 넣었습니다. 그림이 있으면 좋겠다는 반응에도
> 캐릭터는 도구의 성격을 흐린다고 보고 넣지 않았습니다. 마지막에는 앱을 처음 보는 동기에게 주소만 보내
> 휴대폰에서 도움 없이 업체 질문까지 가는 것을 확인했습니다.

근거가 된 결정:

| 날짜 | 내가 정한 것 | 기록 |
|---|---|---|
| 09-21 | 논문 숫자를 보여 주는 앱이 아니라, 받은 성능 숫자에서 빠진 시험 조건을 찾아 공급자 질문으로 바꾸는 판독기로 방향과 설계를 승인했다. 제품을 합격·불합격으로 판정하지 않는다 | `STATUS.md` 09-21, `DESIGN.md` |
| 09-22 | 처음 보는 비전공자에게 공개 주소를 직접 보여 줬고, 목적이 전달되지 않았다는 결과를 숨기지 않고 첫 화면을 고치게 했다. 코드 라이선스는 MIT로 정했다 | `STATUS.md` 09-22, 이 문서 첫 관찰 |
| 09-23 | 결과 1장에 두 번 나오던 근거 패널을 하나로 합치기로 했다. "그림이 있으면 좋겠다"는 반응에도 캐릭터·삽화는 넣지 않았다 | `STATUS.md` 09-23 |
| 09-24 | 동기들의 앱을 보고 새 프로젝트를 만들지 않고 이 앱을 키우기로 했다. "그래서 뭐?" 반응에 나온 세 제안 가운데 결론 카드만 넣었고, 보여 줄 상대를 보안 지식이 어느 정도 있는 사람으로 정했다 | `STATUS.md` 09-24, 이 문서 네 번째 관찰 |
| 09-24 | V10 요청서를 직접 써서 "분석 먼저, 승인 뒤 구현"을 요구했고, 분석에서 나온 수정 7개와 첫 화면 실험실 카드를 한 단계 낮추는 안을 승인했다 | `CLAUDE-V10-IMPROVEMENT-PROMPT.md`, `STATUS.md` 09-24 오후 |
| 09-25 | 독립 검토의 두 후보 가운데 결론이 증거보다 강하게 말하는 오류(후보 1)만 먼저 고쳐 배포했다. 모바일 단추 이동(후보 2)은 동기가 휴대폰에서 도움 없이 질문까지 간 것을 보고 하지 않기로 했다 | `STATUS.md` 09-25, 이 문서 여섯 번째 관찰 |

한 줄 제출안: **앱이 제품을 판정하지 않고 빠진 시험 조건을 업체 질문으로 바꾸게 방향을 정했고, 처음 보는
사람들에게 직접 보여 준 반응을 보고 첫 화면 문장과 결론 카드는 고치되 캐릭터처럼 도구의 성격을 흐리는 요청은
넣지 않았습니다.**

### ③ AI 제안을 따르지 않은 일 — 사실 후보

기록에 "제안했지만 최종 결과물에 들어가지 않은 것"으로 남은 것이다. 이 가운데 무엇을 "내가 따르지 않은
제안"으로 쓸지는 사용자가 정한다. 모두 2026-09-21 사용자가 승인한 설계에서 정해졌다.

| AI 제안 | 결과 | 기록 |
|---|---|---|
| Claude: 설명 안정성(SHAP Top-5 시드 안정성)을 판독 항목 여섯째로 붙인다 | 첫 판에 넣지 않았다. 주 사용자가 SHAP 정보를 가져온다는 가정이 맞지 않고 첫 흐름이 커진다 | `CLAUDE-OPINION.md` 4절 → `DESIGN.md` 7절 |
| Claude: 10번 저장소 대시보드 네 화면을 앱 안 "근거 보기"로 남긴다 | 첫 판 범위에서 뺐다(나중에 선택) | `CLAUDE-OPINION.md` 5절 → `DESIGN.md` 15절 |
| Codex: 이름 "Evidence Gate" | 쓰지 않았다. Gate는 앱이 모델을 승인·거절하는 것처럼 읽힌다 | `CLAUDE-CARD1-REVIEW.md` → `DESIGN.md` 0절 |
| Codex: SplitLens를 사용자가 분할을 넣는 도구로 만든다 | 입력 도구로 만들지 않고 논문의 실제 분할 그림(학습 9종 대 시험 3종)으로만 넣었다 | `CLAUDE-CARD1-ANSWER.md` → `DESIGN.md` 22절 4 |

한 줄 제출안: **Codex가 제안한 `Evidence Gate`는 모델을 승인·거절하는 도구처럼 읽힐 수 있어
따르지 않고, 판정 대신 빠진 평가 조건과 공급자 질문을 드러내는 `ExplainSOC`로 정했다.**

구현 단계(V9 이후)에서 AI 제안을 넣지 않은 기록은 아래와 같다.

| AI 제안 | 결과 | 기록 |
|---|---|---|
| Claude: 비전공 지인의 "그래서 뭐?" 반응에 세 가지(첫 화면 질문형 제목, 결과 결론 카드, 99.88% 예시로 시작)를 제안 | 사용자가 **결론 카드만** 넣기로 했다(2026-09-24). 99.88% 예시로 시작하기는 뒤에 사용자가 직접 쓴 V10 요청서에 따라 V10에서 넣었다 | 이 문서 네 번째 관찰, `DESIGN.md` 31절·32절 |
| Claude: 첫 화면을 전문 용어로 되돌리는 전문가용 첫 화면 | 제안만 했고 사용자가 정하지 않아 넣지 않았다 | `DESIGN.md` 31절, `STATUS.md` 2026-09-24 |
| Codex(V10 독립 검토): 모바일에서 `업체에 물을 질문 보기`를 결론 제목 가까이 옮기는 후보 2 | 동기가 휴대폰에서 도움 없이 질문까지 간 관찰을 근거로 하지 않았다. 검토 문서 스스로도 "관찰에서 문제가 보일 때만"을 조건으로 두었다 | `CODEX-V10-INDEPENDENT-REVIEW-HANDOFF.md` 5절, 이 문서 여섯 번째 관찰 |

이 가운데 무엇을 ③으로 쓸지도 사용자가 정한다.

## 이 카드에서 바꾼 것

| 저장소 | 파일 | 바꾼 것 |
|---|---|---|
| explainsoc | `src/data/paperEvidence.ts`, `README.md` | 데이터셋 인용에서 저자 실명을 빼고 DOI로 가리킨다 |
| explainsoc | `tests/unit/evidence.test.ts` | 인용에 DOI가 있고 저자 표기가 없는지 본다(단위·컴포넌트 74 → 75개) |
| explainsoc | `LICENSE`, `package.json`, `README.md`, `scripts/make-release-zip.mjs` | MIT 라이선스, 묶음 허용 목록에 LICENSE (`2e00f55`) |
| explainsoc | `TROUBLESHOOTING.md`, `README.md` | 포트폴리오용 트러블슈팅 기록과 README 첫머리 링크 (`9d71d39`) |
| explainsoc | `planning/HANDOFF-2026-09-22.md` | 학원 PC에서 이어 갈 인계 문서 |
| explainsoc | `evidence/card-5.md`, `evidence/screenshots/t12-*.png` | 이 문서와 12번 화면 세 장 |
| explainsoc | `src/components/InvestigationBrief.tsx`, `ResultStep.tsx`, `WorkbenchNav.tsx`, `styles/app.css` | V5 결과 우선 브리핑, 독립 3열 카드, 모바일 장면 전환 |
| explainsoc | `tests/`, `planning/DESIGN.md` 27절 | V5 회귀 검사와 설계 결정 (`7d0a105`) |
| explainsoc | `src/components/Hero.tsx`, `styles/app.css`, 첫 화면 테스트 | 비전공자 첫 관찰을 반영한 V6 쉬운 설명과 사용 상황 (`b5384ee`) |
| about (12번) | `content/approved.json`, `docs/index.html`, `docs/assets/t13-claim-flip.webp` | 13번 공개 카드 |
| about (12번) | `device/test_device.py`, `planning/STATUS.md` | 사례 노트 수 검사, 제출 뒤 갱신 기록 |
| about (12번) | `content/approved.json`, `docs/index.html` | 검증 칸 테스트 수 74 → 75 (`ea77b4e`) |
| about (12번) | `content/approved.json`, `docs/index.html`, `docs/assets/t13-decision-brief-v5.png` | V5 대표 그림과 92개·26개 검증 문장 (`e53e2c3`) |
| about (12번) | `content/approved.json`, `docs/index.html` | V6와 같은 쉬운 한 문장 (`64945b4`) |

## 완주 체크리스트

- [x] 12번 사이트의 대표작 자리에 앱을 연결했습니다. — 공개 주소에서 확인했다

## 제출 준비 — 2026-09-25

과제 원문의 제출물 세 가지와 완주 체크리스트를 현재 값으로 맞춘 것이다. 제출 폼에 아래 글을 그대로 옮긴다.

```text
[결과물]
URL: https://myeongjundev.github.io/explainsoc/ (로그인 없이 열림)
ZIP: explainsoc-f34f7d5.zip (SHA-256 9b30bbd2ca626065cb025262fd92775c93b227b619ddfe158b23a724f4903e57)
저장소: https://github.com/myeongjundev/explainsoc

[12번 사이트 대표작 연결]
https://myeongjundev.github.io/about/#work-t13-app

[짧은 확인 방법]
1) 첫 화면의 "예시로 바로 보기"를 누릅니다(숫자 준비 필요 없음).
2) 결론 "광고 숫자만으로는 아직 믿기 이릅니다"와 이유 "99.8점이 38.7점이 됐습니다"가 나오면,
   "업체에 물을 질문 보기"를 눌러 질문 2개와 "왜 묻나요"를 확인하고 "질문만 복사"를 누릅니다.
3) 내 숫자로 해 보려면 "숫자로 직접 입력"에서 정확도 0.99를 적고 조건은 모름으로 두면
   확인 필요 3 · 해석 주의 1 · 질문 4개가 나옵니다. 99를 적어도 멈추지 않고 "99%라면 0.99로 적습니다"가 나옵니다.
ZIP은 압축을 풀고 npm ci → npm run build → npm run preview 후 http://127.0.0.1:4173/explainsoc/ 를 엽니다.

[AI와 나의 판단 세 줄]
① AI에게 맡긴 일: 과제·논문 분석부터 설계 초안, React 구현, 판독 규칙 R01~R15, 접근성·보안 검증,
   Pages 배포와 재현 가능한 ZIP·증거 기록, 완성 뒤 독립 검토까지 Codex와 Claude에게 맡겼습니다.
② 내가 직접 판단한 일: 앱이 제품을 판정하지 않고 빠진 시험 조건을 업체 질문으로 바꾸게 방향을 정했고,
   처음 보는 사람들에게 직접 보여 준 반응을 보고 첫 화면 문장과 결론 카드는 고치되 캐릭터처럼
   도구의 성격을 흐리는 요청은 넣지 않았습니다.
③ AI 제안을 따르지 않은 일: Codex가 제안한 이름 "Evidence Gate"는 앱이 제품을 승인·거절하는 것처럼
   읽혀 쓰지 않았고, 독립 검토가 낸 모바일 질문 단추 이동안은 동기가 휴대폰에서 도움 없이 질문까지
   가는 것을 보고 따르지 않았습니다.
```

| 완주 체크리스트 | 확인 |
|---|---|
| 논문에서 앱이 될 문장 하나와 사용자가 할 일 세 가지 | README "누구를 어떻게 돕는가"·"사용자가 할 일 세 가지", 위 짧은 확인 방법 |
| 첫 화면에 논문 제목과 "누구를 어떻게 돕는가" 한 문장 | 2026-09-25 공개 주소: "이 사이트가 하는 일 …" 문장과 "반영한 논문 · 데이터 분할 방식에 따른 …" |
| 앱이 논문의 결과를 실제로 쓰고, 잘못된 입력에도 멈추지 않음 | 결론·질문·실험실의 수치가 `paperEvidence.ts`에서 옴. 99·abc·-0.2 입력 브라우저 검사 통과 |
| 실제 개인정보와 비밀값을 뺌 | 안전 검사 11개(BRB-C12 절), 필요한 환경 변수 없음 |
| 새 시크릿 창 또는 새 임시 폴더에서 열림 | 저장소 밖 새 임시 폴더에서 ZIP 전체 검사 통과, 공개 주소를 쿠키·로그인 없는 새 브라우저 컨텍스트로 열어 흐름 확인 |
| 12번 사이트 대표작 자리에 앱 연결 | `#work-t13-app` 카드의 앱 체험·검증 기록·공개 저장소 링크를 공개 사이트에서 확인 |
