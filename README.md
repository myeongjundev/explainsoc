...
결과에 들어오면 긴 입력 기록보다 **Decision Brief**가 먼저 나옵니다. 현재 회차에서 확인할 조건,
해석 주의, 다음 질문 수와 가장 먼저 물을 질문을 한눈에 보여 줍니다. 그 아래에서 `주장·근거 → 판독
→ 다음 행동` 순서로 읽고, 회차 기록은 공급자 답변을 이어 붙일 때 펼칩니다. 모바일에서는 세 장을
한 번에 하나씩 전환해 긴 대시보드를 그대로 축소하지 않습니다. 이 요약도 모델의 합격·불합격이나
위험 점수가 아니라 근거 확인 순서입니다.
