...
3. 평가 근거 지도에서 빠진 것을 확인하고, 질문별 답변 상태·메모를 남긴 뒤 PoC 검토표를 복사하거나 PDF로 저장한다.
...
결과에서 나온 판독 규칙 열 개(R01~R10)와 입력 정합성 규칙 세 개(R11~R13)로 사용자가 받은 숫자를
...
| `npm test` | 단위·컴포넌트 테스트 — 계산식, 입력 검증, 판독 규칙 R01~R13, 근거 데이터, 검토표 동작 |
